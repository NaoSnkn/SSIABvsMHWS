...
# Read in the data
as_mipam <- read_xlsx('Data/Aspath_Moderate_Ipam.xlsx', trim_ws=TRUE)
...
# Data distribution
as_mipam |> ggplot(aes(x=FvFm)) + 
  geom_histogram(aes(y=..density..), alpha = .6)+
  geom_density(alpha=.2)

# Look at the data
as_mipam |> ggplot(aes(y = FvFm, x = DHW, color=Genet)) +
  geom_point(position = position_jitter())+
  geom_smooth(aes(x=DHW, y=FvFm)) +
  facet_wrap(~Temperature) +
  theme(legend.position = 'none')

as_mipam |> ggplot(aes(y = FvFm, x = Temperature, color = DHW)) +
...
as_mipam.brm2 <- update(as_mipam.brm,
                         sample_prior = 'yes',
                         refresh = 0,
                         cores = 3, 
                         seed = 123) 
...
# Model diagnostics
as_mipam.brm2$fit |> stan_trace()
as_mipam.brm2$fit |> stan_ac()
as_mipam.brm2$fit |> stan_rhat()
as_mipam.brm2$fit |> stan_ess()
as_mipam.brm2 |> pp_check(type = 'dens_overlay', ndraws = 100)

as_mipam.resids <- make_brms_dharma_res(as_mipam.brm2, integerResponse = FALSE)
testUniformity(as_mipam.resids)
plotResiduals(as_mipam.resids, quantreg = FALSE)
testDispersion(as_mipam.resids)

#### ED50 & ED25 Calculation, Ranking and Visualization ####

# Create a lookup tables that maps Date to Day and DHW
as_mipam.lookup_Day <- tribble(
  ~Date, ~Day,
  "20240320", 0,
  "20240402", 13,
  "20240408", 19)

as_mipam.lookup_DHW <- tribble(
  ~Date, ~DHW,
  "20240320", 0,
  "20240402", 3.30,
  "20240408", 5.97)
...
as_mipam.pred_DHW <- emmeans(as_mipam.brm2, ~ Date | Genet | Temperature, type = "response") |>
  summary() |>
  as_tibble() |>
  left_join(as_mipam.lookup_DHW, by = c("Date" = "Date"))

## ED50s ##

## Calculate the full posteriors of each ED50, including weighted rank and ranks
as_mipam.draws_ED50_DHW <- emmeans(as_mipam.brm2, ~ Date | Genet | Temperature, type = "response") |>
...
as_mipam.medians_ED50_DHW <- as_mipam.draws_ED50_DHW |> 
  group_by(Genet) |> 
...
as_mipam.draws_ED25_DHW <- emmeans(as_mipam.brm2, ~ Date | Genet | Temperature, type = "response") |>
  gather_emmeans_draws() |>
  filter(Temperature == '31.5') |> 
  left_join(as_mipam.lookup_DHW, by = c("Date" = "Date")) |>
  mutate(.value = plogis(.value)) |>
...
as_mipam.medians_ED25_DHW <- as_mipam.draws_ED25_DHW |> 
  group_by(Genet) |> 
...
as_mipam_ED50s_DHW <- as_mipam.medians_ED50_DHW |>
  mutate(Genet = factor(Genet, levels=as_mipam.medians_ED50_DHW |>  arrange(ED50) |> pull(Genet))) |> 
...
as_mipam_ED25s_DHW <- as_mipam.medians_ED25_DHW |>
  mutate(Genet = factor(Genet, levels=as_mipam.medians_ED25_DHW |>  arrange(ED25) |> pull(Genet))) |> 
...
# Get the predicted values for the response
as_mipam.pred_Day <- emmeans(as_mipam.brm2, ~ Date | Genet | Temperature, type = "response") |>
  summary() |>
  as_tibble() |>
  left_join(as_mipam.lookup_Day, by = c("Date" = "Date"))

## ED50s ##

# Calculate the full posteriors of each ED50, including weighted rank and ranks
as_mipam.draws_ED50_Day <- emmeans(as_mipam.brm2, ~ Date | Genet | Temperature, type = "response") |>
  gather_emmeans_draws() |>
  filter(Temperature == '31.5') |> 
  left_join(as_mipam.lookup_Day, by = c("Date" = "Date")) |>
  mutate(.value = plogis(.value)) |>
...
# Calculate the median for ED50s, Weighted _ranks, and Ranks draws
as_mipam.medians_ED50_Day <- as_mipam.draws_ED50_Day |> 
  group_by(Genet) |> 
  summarise(ED50_median = median(ED50, na.rm=TRUE),
            ED50_lower = HDInterval::hdi(ED50)[1],
            ED50_upper = HDInterval::hdi(ED50)[2]) |> 
...
# Calculate the full posteriors of each ED25, including weighted rank and ranks
as_mipam.draws_ED25_Day <- emmeans(as_mipam.brm2, ~ Date | Genet | Temperature, type = "response") |>
  gather_emmeans_draws() |>
  filter(Temperature == '31.5') |> 
  left_join(as_mipam.lookup_Day, by = c("Date" = "Date")) |>
  mutate(.value = plogis(.value)) |>
...
# Calculate the median for ED25s, Weighted _ranks, and Ranks draws
as_mipam.medians_ED25_Day <- as_mipam.draws_ED25_Day |> 
  group_by(Genet) |> 
  summarise(ED25_median = median(ED25, na.rm=TRUE),
            ED25_lower = HDInterval::hdi(ED25)[1],
            ED25_upper = HDInterval::hdi(ED25)[2]) |> 
...
# Plot medians of ED50s
as_mipam_ED50s_Day <- as_mipam.medians_ED50_Day |>
  mutate(Genet = factor(Genet, levels=as_mipam.medians_ED50_Day |>  arrange(ED50) |> pull(Genet))) |> 
  ggplot(aes(x = ED50, y = Genet)) +
  geom_pointrange(aes(xmin = ED50_lower, xmax = ED50_upper)) +
  labs(x = "Effective Dose at 50% (C°)") +
  theme_light()
...
# Plot medians of ED25s
as_mipam_ED25s_Day <- as_mipam.medians_ED25_Day |>
  mutate(Genet = factor(Genet, levels=as_mipam.medians_ED25_Day |>  arrange(ED25) |> pull(Genet))) |> 
  ggplot(aes(x = ED25, y = Genet)) +
  geom_pointrange(aes(xmin = ED25_lower, xmax = ED25_upper)) +
  labs(x = "Effective Dose at 25% (C°)") +
  theme_light()
...
as_mipam_EDs_medians_DHW <- as_mipam_ED50s_DHW + as_mipam_ED25s_DHW + 
  plot_annotation(tag_levels = 'A')
as_mipam_EDs_medians_Day <- as_mipam_ED50s_Day + as_mipam_ED25s_Day + 
  plot_annotation(tag_levels = 'A')

### FINAL PLOT ###

as_mipam_EDs_perGenet <- as_mipam.pred_DHW |> 
  mutate(Genet = factor(Genet, levels=as_mipam.medians_ED25_DHW |>  arrange(desc(ED25)) |> pull(Genet))) |> 
  ...
  geom_vline(data = as_mipam.medians_ED25_DHW, aes(xintercept=ED25, color="ED25"),linewidth=0.75) +
  geom_vline(data = as_mipam.medians_ED25_DHW, aes(xintercept=ED25_lower, color="ED25"), alpha = 0.3) +
  geom_vline(data = as_mipam.medians_ED25_DHW, aes(xintercept=ED25_upper, color="ED25"), alpha = 0.3) +
  geom_rect(data = as_mipam.medians_ED25_DHW, 
            aes(xmin = ED25_lower, xmax = ED25_upper, ymin = -Inf, ymax = Inf, fill="ED25"), 
            alpha = 0.2, inherit.aes = FALSE)+
  geom_vline(data = as_mipam.medians_ED50_DHW, aes(xintercept=ED50, color = "ED50"),linewidth=0.75) +
  geom_vline(data = as_mipam.medians_ED50_DHW, aes(xintercept=ED50_lower, color = "ED50"), alpha = 0.3) +
  geom_vline(data = as_mipam.medians_ED50_DHW, aes(xintercept=ED50_upper, color = "ED50"), alpha = 0.3) +
  geom_rect(data = as_mipam.medians_ED50_DHW, 
