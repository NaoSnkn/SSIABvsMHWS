...
as_aipam <- read_xlsx('Data/Aspath_Acute_Ipam.xlsx', trim_ws=TRUE)
...
# Data distribution
as_aipam |> ggplot(aes(x=FvFm)) + 
  geom_histogram(aes(y=..density..)) +
  geom_density(alpha=.2)

# Look at the data
as_aipam |> ggplot(aes(y = FvFm, x = Treatment)) +
...
# Let's remove genets that are not found in MMM10 at all and unusable values
as_aipam <- as_aipam |> 
  filter(!(Genet %in% c('1515', '1566', '801', '998'))) |> 
  filter(FvFm > 0.00 & FvFm < 0.75)
...
# Formula
as_aipam.form <- bf(FvFm ~ Treatment*Genet,
                     phi = ~Treatment,
                     family = Beta(link = "logit"))

# Help defining priors if needed
as_aipam |> group_by(Treatment) |> 
  summarise(median(logit(FvFm), na.rm = TRUE),
            mad(logit(FvFm), na.rm = TRUE))

# Priors
priors <- prior(normal(0.5,0.5), class = 'Intercept') +  
  prior(normal(0,1.5), class = 'b') +
  prior(gamma(4, 0.8), class = 'Intercept', dpar = 'phi', lb=0) +
  prior(normal(0,1), class = 'b', dpar = 'phi') 

# Run model with sample priors only
as_aipam.brm2 <- brm(as_aipam.form, 
                     data = as_aipam,
                     prior = priors,
                     sample_prior = 'yes',
                     iter = 15000,
...
as_aipam.brm2 <- update(as_aipam.brm,
                         sample_prior = 'yes',
                         refresh = 0,
                         cores = 3, 
                         seed = 123) 
...
# Model diagnostics
as_aipam.brm2$fit |> stan_trace()
as_aipam.brm2$fit |> stan_ac() 
as_aipam.brm2$fit |> stan_rhat()
as_aipam.brm2$fit |> stan_ess()
as_aipam.brm2 |> pp_check(type = 'dens_overlay', ndraws = 100)

as_aipam.resids <- make_brms_dharma_res(as_aipam.brm2, integerResponse = FALSE)
testUniformity(as_aipam.resids)
plotResiduals(as_aipam.resids, quantreg = FALSE)
testDispersion(as_aipam.resids)


#### ED50 & ED25 Calculation, Ranking and Visualization ####

# Create a lookup table that maps Treatments to Temperature
as_aipam.lookup <- tribble(
...
as_aipam.pred <- emmeans(as_aipam.brm2, ~ Treatment | Genet, type = "response") |>
  summary() |>
  as_tibble() |>
  left_join(as_aipam.lookup, by = c("Treatment" = "Treatment")) 
...
as_aipam.draws_ED50 <- emmeans(as_aipam.brm2, ~ Treatment | Genet, type = "response") |>
...
as_aipam.medians_ED50 <- as_aipam.draws_ED50 |> 
  group_by(Genet) |> 
...
as_aipam.draws_ED25 <- emmeans(as_aipam.brm2, ~ Treatment | Genet, type = "response") |>
  gather_emmeans_draws() |>
  left_join(as_aipam.lookup, by = c("Treatment" = "Treatment")) |>
  mutate(.value = plogis(.value)) |>
  arrange(Temperature) |>
...
as_aipam.medians_ED25 <- as_aipam.draws_ED25 |> 
  group_by(Genet) |> 
...
as_aipam_ED50s <- as_aipam.medians_ED50 |>
  mutate(Genet = factor(Genet, levels=as_aipam.medians_ED50 |>  arrange(ED50) |> pull(Genet))) |> 
...
as_aipam_ED25s <- as_aipam.medians_ED25 |>
  mutate(Genet = factor(Genet, levels=as_aipam.medians_ED25 |>  arrange(ED25) |> pull(Genet))) |> 
...
as_aipam_EDs_medians <- as_aipam_ED50s + as_aipam_ED25s + 
  plot_annotation(tag_levels = 'A')

### FINAL PLOT ###

as_aipam_EDs_perGenet <- as_aipam.pred |> 
  mutate(Genet = factor(Genet, levels=as_aipam.medians_ED50 |>  arrange(desc(ED50)) |> pull(Genet))) |> 
  filter(Genet %in% common_genets) |> 
  filter(Genet != 'NA') |> 
  ...
  geom_vline(data = as_aipam.medians_ED25, aes(xintercept=ED25, color="ED25"),linewidth=0.75) +
  geom_vline(data = as_aipam.medians_ED25, aes(xintercept=ED25_lower, color="ED25"), alpha = 0.3) +
  geom_vline(data = as_aipam.medians_ED25, aes(xintercept=ED25_upper, color="ED25"), alpha = 0.3) +
  geom_rect(data = as_aipam.medians_ED25, 
            aes(xmin = ED25_lower, xmax = ED25_upper, ymin = -Inf, ymax = Inf, fill="ED25"), 
            alpha = 0.2, inherit.aes = FALSE)+
  geom_vline(data = as_aipam.medians_ED50, aes(xintercept=ED50, color = "ED50"),linewidth=0.75) +
  geom_vline(data = as_aipam.medians_ED50, aes(xintercept=ED50_lower, color = "ED50"), alpha = 0.3) +
  geom_vline(data = as_aipam.medians_ED50, aes(xintercept=ED50_upper, color = "ED50"), alpha = 0.3) +
  geom_rect(data = as_aipam.medians_ED50, 
