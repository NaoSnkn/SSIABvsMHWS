...
ak_ahyper <- read_xlsx('Data/Akenti_Acute_Hyp.xlsx', trim_ws=TRUE)
...
# Formula
ak_ahyper.form <- bf(MeanNDVI ~ Treatment*Genet,
                      phi = ~Treatment,
                      family = Beta(link = "logit"))

# Help defining priors if needed
ak_ahyper |> group_by(Treatment) |> 
...
ak_ahyper.brm <- brm(ak_ahyper.form, 
                    data = ak_ahyper,
                    prior = priors,
                    sample_prior = 'only',
                    iter = 10000,
...
ak_ahyper.brm2 <- update(ak_ahyper.brm,
                       sample_prior = 'yes',
                       refresh = 0,
                       cores = 3, 
                       seed = 123) 
...
# Model diagnostics
ak_ahyper.brm2$fit |> stan_trace()
ak_ahyper.brm2$fit |> stan_ac()
ak_ahyper.brm2$fit |> stan_rhat()
ak_ahyper.brm2$fit |> stan_ess()
ak_ahyper.brm2 |> pp_check(type = 'dens_overlay', ndraws = 100)

ak_ahyper.resids <- make_brms_dharma_res(ak_ahyper.brm2, integerResponse = FALSE)
testUniformity(ak_ahyper.resids)
plotResiduals(ak_ahyper.resids, quantreg = FALSE)
testDispersion(ak_ahyper.resids)


#### ED50 & ED25 Calculation, Ranking and Visualization ####

# Create a lookup table that maps Treatments to Temperature
ak_ahyper.lookup <- tribble(
...
common_genets <- ak_ahyper.brm2[[2]] |> 
...
ak_ahyper.pred <- emmeans(ak_ahyper.brm2, ~ Treatment | Genet, type = "response") |>
  summary() |>
  as_tibble() |>
  left_join(ak_ahyper.lookup, by = c("Treatment" = "Treatment")) 
...
ak_ahyper.draws_ED50 <- emmeans(ak_ahyper.brm2, ~ Treatment | Genet, type = "response") |>
...
ak_ahyper.medians_ED50 <- ak_ahyper.draws_ED50 |> 
  group_by(Genet) |> 
...
ak_ahyper.draws_ED25 <- emmeans(ak_ahyper.brm2, ~ Treatment | Genet, type = "response") |>
  gather_emmeans_draws() |>
  left_join(ak_ahyper.lookup, by = c("Treatment" = "Treatment")) |>
  mutate(.value = plogis(.value)) |>
  arrange(Temperature) |>
...
ak_ahyper.medians_ED25 <- ak_ahyper.draws_ED25 |> 
  group_by(Genet) |> 
...
ak_ahyper_ED50s <- ak_ahyper.medians_ED50 |>
  mutate(Genet = factor(Genet, levels=ak_ahyper.medians_ED50 |>  arrange(ED50) |> pull(Genet))) |> 
...
ak_ahyper_ED25s <- ak_ahyper.medians_ED25 |>
  mutate(Genet = factor(Genet, levels=ak_ahyper.medians_ED25 |>  arrange(ED25) |> pull(Genet))) |> 
...
ak_ahyper_EDs_medians <- ak_ahyper_ED50s + ak_ahyper_ED25s + 
  plot_annotation(tag_levels = 'A')

### FINAL PLOT ###

ak_ahyper_EDs_perGenet <- ak_ahyper.pred |> 
  mutate(Genet = factor(Genet, levels=ak_ahyper.medians_ED50 |>  arrange(desc(ED50)) |> pull(Genet))) |> 
  filter(Genet %in% common_genets) |>  
  ggplot(aes(y = response, x = Temperature)) +
  facet_wrap(~Genet)+
  labs(y = "Mean NDVI", x ="Temperature (C°)") +
  geom_line(linewidth=0.75, lineend = "round", color = "#696969") +
  geom_vline(data = ak_ahyper.medians_ED25, aes(xintercept=ED25, color="ED25"),linewidth=0.75) +
  geom_vline(data = ak_ahyper.medians_ED25, aes(xintercept=ED25_lower, color="ED25"), alpha = 0.3) +
  geom_vline(data = ak_ahyper.medians_ED25, aes(xintercept=ED25_upper, color="ED25"), alpha = 0.3) +
  geom_rect(data = ak_ahyper.medians_ED25, 
            aes(xmin = ED25_lower, xmax = ED25_upper, ymin = -Inf, ymax = Inf, fill="ED25"), 
            alpha = 0.2, inherit.aes = FALSE)+
  geom_vline(data = ak_ahyper.medians_ED50, aes(xintercept=ED50, color = "ED50"),linewidth=0.75) +
  geom_vline(data = ak_ahyper.medians_ED50, aes(xintercept=ED50_lower, color = "ED50"), alpha = 0.3) +
  geom_vline(data = ak_ahyper.medians_ED50, aes(xintercept=ED50_upper, color = "ED50"), alpha = 0.3) +
  geom_rect(data = ak_ahyper.medians_ED50, 
