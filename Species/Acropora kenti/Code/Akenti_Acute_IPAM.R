...
# read in Data 
ak_aipam <- read_xlsx('Data/Akenti_Acute_Ipam.xlsx', trim_ws=TRUE)
...
# Data distribution
ak_aipam |> ggplot(aes(x=FvFm)) + 
    geom_histogram(aes(y=..density..)) +
    geom_density(alpha=.2)

# Look at the data
ak_aipam |> ggplot(aes(y = FvFm, x = Treatment)) +
...
# Formula
ak_aipam.form <- bf(FvFm ~ Treatment*Genet,
                     phi = ~Treatment,
                     family = Beta(link = "logit"))

# Remove under and oversaturated values
ak_aipam <- ak_aipam |>
  filter(!is.na(FvFm)) |> 
  filter(FvFm > 0.00 & FvFm < 0.75)

# Help defining priors if needed
ak_aipam |> group_by(Treatment) |> 
...
ak_aipam.brm2 <- brm(ak_aipam.form,
                    data = ak_aipam,
                    prior = priors,
                    phi ~ Treatment,
                    sample_prior = 'yes',
...
ak_aipam.brm2 <- update(ak_aipam.brm,
                       sample_prior = 'yes',
                       refresh = 0,
                       cores = 3, 
                       seed = 123) 
...
# Model diagnostics
ak_aipam.brm2$fit |> stan_trace()
ak_aipam.brm2$fit |> stan_ac()
ak_aipam.brm2$fit |> stan_rhat()
ak_aipam.brm2$fit |> stan_ess()
ak_aipam.brm2 |> pp_check(type = 'dens_overlay', ndraws = 100)

ak_aipam.resids <- make_brms_dharma_res(ak_aipam.brm2, integerResponse = FALSE)
testUniformity(ak_aipam.resids)
plotResiduals(ak_aipam.resids, quantreg = FALSE)
testDispersion(ak_aipam.resids)

#### ED50 & ED25 Calculation, Ranking and Visualization ####

# Create a lookup table that maps Treatments to Temperature
ak_aipam.lookup <- tribble(
...
ak_aipam.pred <- emmeans(ak_aipam.brm2, ~ Treatment | Genet, type = "response") |>
  summary() |>
  as_tibble() |>
  left_join(ak_aipam.lookup, by = c("Treatment" = "Treatment")) 
...
ak_aipam.draws_ED50 <- emmeans(ak_aipam.brm2, ~ Treatment | Genet, type = "response") |>
...
ak_aipam.medians_ED50 <- ak_aipam.draws_ED50 |> 
  group_by(Genet) |> 
...
ak_aipam.draws_ED25 <- emmeans(ak_aipam.brm2, ~ Treatment | Genet, type = "response") |>
  gather_emmeans_draws() |>
  left_join(ak_aipam.lookup, by = c("Treatment" = "Treatment")) |>
  mutate(.value = plogis(.value)) |>
  arrange(Temperature) |>
...
ak_aipam.medians_ED25 <- ak_aipam.draws_ED25 |> 
  group_by(Genet) |> 
...
ak_aipam_ED50s <- ak_aipam.medians_ED50 |>
  mutate(Genet = factor(Genet, levels=ak_aipam.medians_ED50 |>  arrange(ED50) |> pull(Genet))) |> 
...
ak_aipam_ED25s <- ak_aipam.medians_ED25 |>
  mutate(Genet = factor(Genet, levels=ak_aipam.medians_ED25 |>  arrange(ED25) |> pull(Genet))) |> 
...
ak_aipam_EDs_medians <- ak_aipam_ED50s + ak_aipam_ED25s + 
  plot_annotation(tag_levels = 'A')

### FINAL PLOT ###

ak_aipam_EDs_perGenet <- ak_aipam.pred |> 
  mutate(Genet = factor(Genet, levels=ak_aipam.medians_ED50 |>  arrange(desc(ED50)) |> pull(Genet))) |> 
  filter(Genet %in% common_genets) |> 
  filter(Genet != 'NA') |> 
  ...
  geom_vline(data = ak_aipam.medians_ED25, aes(xintercept=ED25, color="ED25"),linewidth=0.75) +
  geom_vline(data = ak_aipam.medians_ED25, aes(xintercept=ED25_lower, color="ED25"), alpha = 0.3) +
  geom_vline(data = ak_aipam.medians_ED25, aes(xintercept=ED25_upper, color="ED25"), alpha = 0.3) +
  geom_rect(data = ak_aipam.medians_ED25, 
            aes(xmin = ED25_lower, xmax = ED25_upper, ymin = -Inf, ymax = Inf, fill="ED25"), 
            alpha = 0.2, inherit.aes = FALSE)+
  geom_vline(data = ak_aipam.medians_ED50, aes(xintercept=ED50, color = "ED50"),linewidth=0.75) +
  geom_vline(data = ak_aipam.medians_ED50, aes(xintercept=ED50_lower, color = "ED50"), alpha = 0.3) +
  geom_vline(data = ak_aipam.medians_ED50, aes(xintercept=ED50_upper, color = "ED50"), alpha = 0.3) +
  geom_rect(data = ak_aipam.medians_ED50, 
