...
# If wanting to load the models later: brm2_models_DHW <- readRDS('Models/ah_surv.brm2_list_DHW.rds')
...
generate_ED50_draws_only <- function(model_list) {
  ...
  ah_surv.medians_ED50_draws <- ah_surv.medians_ED50_draws |> 
    ...
    mutate(
      Rank = rank(desc(ED50)),
      Weighted_rank = (max(ED50) - ED50) * 26 / (max(ED50) - min(ED50)) + 1
    ) |>
    ungroup()
...
generate_ED25_draws_only <- function(model_list) {
  ...
  ah_surv.medians_ED25_draws <- ah_surv.medians_ED25_draws |> 
    ...
    mutate(
      Rank = rank(desc(ED25)),
      Weighted_rank = (max(ED25) - ED25) * 26 / (max(ED25) - min(ED25)) + 1
    ) |>
    ungroup()
